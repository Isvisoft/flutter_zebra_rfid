...
//    TextView textView;
    ...
    private FlutterZebraRfidPlugin flutterClass;
    // general
    ...
    void onCreate(FlutterZebraRfidPlugin activity, Context c) {
        // application context
        flutterClass = activity;
        context = c;
        // SDK
        InitSDK();
    }
...
//                });
            ...
            if (rfidStatusEvents.StatusEventData.getStatusEventType() == STATUS_EVENT_TYPE.HANDHELD_TRIGGER_EVENT) {
                if (rfidStatusEvents.StatusEventData.HandheldTriggerEventData.getHandheldEvent() == HANDHELD_TRIGGER_EVENT_TYPE.HANDHELD_TRIGGER_PRESSED) {
                    new AsyncTask<Void, Void, Void>() {
                        @Override
                        protected Void doInBackground(Void... voids) {
                            flutterClass.handleTriggerPress(true);
                            return null;
                        }
                    }.execute();
                }
                if (rfidStatusEvents.StatusEventData.HandheldTriggerEventData.getHandheldEvent() == HANDHELD_TRIGGER_EVENT_TYPE.HANDHELD_TRIGGER_RELEASED) {
                    new AsyncTask<Void, Void, Void>() {
                        @Override
                        protected Void doInBackground(Void... voids) {
                            flutterClass.handleTriggerPress(false);
                            return null;
                        }
                    }.execute();
                }
    ...
    private class AsyncDataUpdate extends AsyncTask<TagData[], Void, Void> {
        ...
        protected Void doInBackground(TagData[]... params) {
            flutterClass.handleTagdata(params[0]);
            return null;
        }
    }
